...
function smarty_function_thumb($params, &$smarty) {/*{{{*/
	...
	$params['dev'] = isset($params['dev']);   
	$params['shortside'] = isset($params['shortside']);    
	$params['longside'] = isset($params['longside']);    
	if (!isset($params['width'])) { $params['width'] = ''; }   
	if (!isset($params['height']))  { $params['height'] = ''; } 
	if (!isset($params['offset'])) { $params['offset'] = ''; }   
	if (!isset($params['sharpen']))  { $params['sharpen'] = ''; } 
  ...
  //  print_r($params);
	...
	if (empty($params['file'])) { trigger_error("thumb: parameter 'file' cannot be empty");return; }
	if (!file_exists($params['file'])) { trigger_error("thumb: image file does not exist");return; }
	...
	if (empty($params['address'])) $params['address'] = false;     // by Pride      
    /*}}}*/
	...
	if (!empty($params['frame'])) {
        ...
        if ($imagesize[0] != $imagesize[1] OR $imagesize[0]%3 OR !file_exists($params['frame'])) { 
            trigger_error("thumb: wrong dimensions of 'frame'-image or width and height is not a multiplier of 3"); return; 
        }
	...
	if ($params['address']) {
		$returner = 'http://'.$_SERVER['SERVER_NAME'].'/'.$_DST['file'];
	} else {
	
		if (empty($params['html'])) $_RETURN['img'] = '<img src="http://'.$_SERVER['SERVER_NAME'].'/'.$_DST['file'].'" '.$params['html'].' '.$_DST['string'].' alt="" title="" />';
		else $_RETURN['img'] = '<img src="http://'.$_SERVER['SERVER_NAME'].'/'.$_DST['file'].'" '.$params['html'].' '.$_DST['string'].' />';

		if ($params['link'] == "true") {
			if (empty($params['linkhtml'])) { $linkhtml = ""; } else { $linkhtml = " ".$params['linkhtml'];}; 
				
			if (empty($params['linkurl'])) $params['linkurl'] = $_SRC['file'];
			
			if ($params['window'] == "true") $returner = '<a href="'.$params['linkurl'].'" target="_blank"'.$linkhtml.'>'.$_RETURN['img'].'</a>';
			else $returner = '<a href="'.$params['linkurl'].'"'.$linkhtml.'>'.$_RETURN['img'].'</a>';
	    } else {
